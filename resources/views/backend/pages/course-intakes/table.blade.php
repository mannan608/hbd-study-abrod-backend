    ...
    <div x-show="showDeleteModal" x-cloak class="fixed inset-0 z-[99999]">
        ...
        <div class="absolute inset-0 flex items-center justify-center p-4">
            <div
                ...
                <div class="p-5">
                    <div class="text-base font-semibold text-gray-800 dark:text-white/90">Delete Course Intake?</div>
        ...
        <div class="max-w-full overflow-x-auto">
            <table class="w-full text-left border-collapse">
                <thead class="bg-gray-50 dark:bg-white/[0.02] border-b border-gray-100 dark:border-white/[0.05]">
                    <tr>
                        <th class="px-5 py-4 text-xs font-medium text-gray-500 uppercase">ID</th>
                        <th class="px-5 py-4 text-xs font-medium text-gray-500 uppercase">Course</th>
                        <th class="px-5 py-4 text-xs font-medium text-gray-500 uppercase">Intake</th>
                        <th class="px-5 py-4 text-xs font-medium text-gray-500 uppercase">Deadline</th>
                        <th class="px-5 py-4 text-xs font-medium text-gray-500 uppercase">Start Date</th>
                        <th class="px-5 py-4 text-xs font-medium text-gray-500 uppercase">Status</th>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-white/[0.05]">
                    ...
                    <template x-for="row in tableRowData" :key="row.id">
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-5 py-4">
                                <span class="px-2 py-1 bg-gray-100 rounded text-xs font-mono" x-text="row.id"></span>
                            </td>
                            <td class="px-5 py-4 text-sm text-gray-700" x-text="row.course_title"></td>
                            <td class="px-5 py-4 text-sm text-gray-700"
                                x-text="`${row.intake_month} ${row.intake_year}`"></td>
                            <td class="px-5 py-4 text-sm text-gray-700" x-text="row.application_deadline"></td>
                            <td class="px-5 py-4 text-sm text-gray-700" x-text="row.start_date || 'N/A'"></td>
                            <td class="px-5 py-4 text-sm text-gray-700 capitalize" x-text="row.status"></td>
                            <td class="px-5 py-4">
