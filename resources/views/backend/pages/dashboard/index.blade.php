...
@section('content')
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-neutral-500 dark:text-neutral-400">Welcome back, here's what's happening today.</p>
        </div>
        <span class="rounded-lg bg-neutral-100 px-3 py-1 text-sm text-neutral-600 dark:bg-neutral-800 dark:text-neutral-300">
            {{ now()->format('l, F j, Y') }}
        </span>
    </div>
    ...
    <div class="grid gap-6 lg:grid-cols-3">
        ...
        <div class="lg:col-span-2">
            <div class="rounded-xl border border-neutral-200 bg-white dark:border-neutral-700 dark:bg-neutral-800">
                <div class="flex items-center justify-between border-b border-neutral-100 px-6 py-4 dark:border-neutral-700">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left text-sm">
                        <thead class="bg-neutral-50 text-xs uppercase text-neutral-500 dark:bg-neutral-700/50 dark:text-neutral-400">
                            <tr>
                                ...
                                <tr class="hover:bg-neutral-50 dark:hover:bg-neutral-700/50">
                                    <td class="px-6 py-3">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <span class="font-medium text-neutral-900 dark:text-white">{{ $student->user->name ?? 'Unknown' }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-3 text-neutral-500 dark:text-neutral-400">{{ $student->user->email ?? '-' }}</td>
                                    <td class="px-6 py-3">
                                        <span class="line-clamp-1 rounded-md bg-neutral-100 px-2 py-1 text-xs font-medium text-neutral-700 dark:bg-neutral-700 dark:text-neutral-300">
                                            {{ $student->courses->first()->name ?? 'N/A' }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-3 text-neutral-500 dark:text-neutral-400">{{ $student->created_at?->diffForHumans() ?? '-' }}</td>                                   
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="5" class="px-6 py-8 text-center text-neutral-500 dark:text-neutral-400">
                                        <svg class="mx-auto mb-2 h-8 w-8 text-neutral-300 dark:text-neutral-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/>
                                        </svg>
        ...
        <div class="space-y-6">
                    ...
                    @forelse ($topCourses ?? [] as $course)
                        <div>
                            <div class="mb-1 flex items-center justify-between text-sm">
                                ...
                                <span class="font-medium text-neutral-900 dark:text-white">{{ $course->students_count ?? 0 }}</span>
                            </div>
            ...
            <div class="rounded-xl border border-neutral-200 bg-white p-5 dark:border-neutral-700 dark:bg-neutral-800">
                ...
                <div class="space-y-2">
                    <a href="#" class="flex items-center gap-3 rounded-lg border border-neutral-200 p-3 transition-colors hover:bg-neutral-50 dark:border-neutral-700 dark:hover:bg-neutral-700/50">
